...
- 函数返回 `error` 时，把 `return nil` 写成独立分支——**永远不要让一个 typed nil 变量流到 return 语句**。
- 看到 `err != nil` 通过但 `err.Error()` panic，第一反应就查上游是否返回了 typed nil。
- 定义接口之前先问自己"实现者会不会传值进来"——如果会，方法必须用值接收者，否则编译不过。
- 类型断言一律写 `v, ok :=` 两返回值形式，把它当作团队代码规范来执行。
- 错误分类用 `errors.As`，不要裸断言——这样未来加 `%w` 包装层也不会悄悄破坏判断逻辑。
