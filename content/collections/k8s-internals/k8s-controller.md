...
这就是 Controller 存在的意义：**它是声明式系统的执行引擎，负责不断地将现实状态调谐到期望状态**。
...
2. **限速**：RateLimitingQueue 的指数退避防止 Controller 在故障时打爆 API Server
...
### 场景 2：200+ 集群失联，Reconcile 风暴打爆 API Server
...
> **用最少的 API Server 交互，实现最终一致的状态调谐。**
