...
**Service** 就是 K8s 给出的答案：提供一个稳定的虚拟 IP（ClusterIP），背后自动关联一组 Pod，并做负载均衡。负载均衡策略取决于 kube-proxy 的模式：iptables 模式（默认）是**随机**分配（基于概率匹配，不记录上一次发给了谁），IPVS 模式默认是**轮询（Round Robin）**，还支持最少连接（lc）、源地址哈希（sh）等多种算法。
...
答案是 **Endpoints**。每创建一个 Service，Kubernetes 会自动创建一个同名的 Endpoints 对象。Endpoints Controller 持续 watch Service 和 Pod：
...
**Endpoints 的问题和 EndpointSlice 的引入**

Endpoints 有一个设计缺陷：它把一个 Service 关联的**所有** Pod IP 塞进同一个 API 对象中。在小集群里没什么问题，但在大规模集群中（一个 Service 关联数千个 Pod），每次有一个 Pod 变化（扩容、重启、健康检查失败），整个 Endpoints 对象都要全量更新并推送给所有节点的 kube-proxy。

**EndpointSlice**（K8s 1.21+ 默认启用）是 Endpoints 的替代方案，解决了这个扩展性问题：

- 把端点**分片**存储，每个 slice 默认最多 100 个端点
- 单个 Pod 变化只需更新它所在的那个 slice，不影响其他 slice
- 更新仍然会推送给所有节点的 kube-proxy（kube-proxy 需要完整的后端列表来配规则），但优化的是**每次传输的数据量**——比如 2000 个 Pod 分成 20 个 slice，一个 Pod 变化只传 100 条记录而不是 2000 条，API Server 传输量和 etcd 写入量都大幅减少

两者的关系：**EndpointSlice 是 Endpoints 的演进替代**，不是两个独立的功能。目前 Kubernetes 会同时维护两者（向后兼容），但新版本的 kube-proxy 已经默认 watch EndpointSlice 而不是 Endpoints。Endpoints API 未来可能被废弃。
...
IPVS 模式使用内核的 IPVS（IP Virtual Server）模块，针对性地解决了 iptables 的两个问题：

| | iptables | IPVS |
|---|---|---|
| 查找复杂度 | O(n) 线性遍历规则链 | O(1) 哈希表查找 |
| 更新方式 | 全量替换所有规则 | 增量更新，只改变化的条目 |
| 调度算法 | 随机（概率匹配） | 轮询（rr）、最少连接（lc）、源地址哈希（sh）等 |
